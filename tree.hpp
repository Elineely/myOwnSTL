...
namespace ft
{
	...
	private:
		...
		Compare			comp_;
		node_type*		root_;
		// 왼쪽 자식에 루트를 갖고있는 태초의 노드
		node_type*		end_;
		int				size_;
		// 이것저것(plus, minus등등의 functor가 들어왔을때 std와 똑같이 동작시켜주기 위해)
		bool			comp_factor;
	...
	private:
		...
		// destroy node and all the chilendren of it.
		// does not change the size. That's the job in "clear()".
	...
	public:
		// if the key of pair is already exist in the tree, do nothing.
		node_type*	insert(value_type pair)
		{
			if (this->_search(this->root_, pair.first) != this->end_)
				return (NULL);
			node_type* newnode = _make_node(pair);
		...
		void	remove(value_type pair)
		{
			this->root_ = _remove(this->root_, pair);
		...
		node_type* __lower_bound(node_type* temp, key_type val) const
		{
			node_type* node;

			if (val == temp->pair_data_.first)
				return (temp);
			else if (this->comp_(val, temp->pair_data_.first))
			{
				if (temp->left_ == NULL)
					return (temp);
				node = __lower_bound(temp->left_, val);
			}
			else
			{
				if (temp->right_ == NULL)
					return (successor(temp));
				node = __lower_bound(temp->right_, val);
			}
			return (node);
		};

		node_type* lower_bound(key_type val) const
		{ return __lower_bound(this->root_, val); };

		node_type* __upper_bound(node_type* temp, key_type val) const
		{
			node_type* node;

			if (this->comp_(val, temp->pair_data_.first))
			{
				if (temp->left_ == NULL)
					return (temp);
				node = __upper_bound(temp->left_, val);
			}
			else
			{
				if (temp->right_ == NULL)
					return (successor(temp));
				node = __upper_bound(temp->right_, val);
			}
			return (node);
		};

		node_type* upper_bound(key_type val) const
		{ return __upper_bound(this->root_, val); };
