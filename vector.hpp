...
namespace ft
{
	...
	class vector_base : protected vector_base_common<true>
	{
	public:
	...
	protected:
		...
		vector_base() throw() : begin_(NULL), end_(NULL), end_cap_(NULL, allocator_type()) {}
		vector_base(const allocator_type& a) : begin_(NULL), end_(NULL), end_cap_(NULL, a) {}
		~vector_base()
		{
			if (begin_ != NULL)
			{
		...
		void					__copy_data(iterator const& new_begin_, iterator const& new_end,
											pair<iterator, Allocator> const& new_end_cap_) throw();

		void 					clear() throw() { __destroy_from_end(begin_); }
		size_type 				capacity() const throw() { return static_cast<size_type>(end_cap() - begin_); }
		allocator_type&			alloc() throw() { return end_cap_.second; }
		const allocator_type&	alloc() const throw() { return end_cap_.second; }
		pointer&				end_cap() throw() { return end_cap_.first; }
		const pointer&			end_cap() const throw() { return end_cap_.first; }
	...
	// vector class
	template <typename Tp, typename Allocator = std::allocator<Tp> >
	class vector : private vector_base<Tp, Allocator>
	{
	private:
	...
	public:
		...
		vector() throw() {}
		explicit vector(const allocator_type& a) throw() : base(a) {}
		explicit vector(size_type n);
		vector(size_type n, const value_type& x);
		vector(size_type n, const value_type& x, const allocator_type& a);
		template <typename InputIterator>
		vector(InputIterator first,
				typename enable_if<(is_input_iterator<InputIterator>::value && !(is_forward_iterator<InputIterator>::value)),
		...
		void swap(vector<Tp, Allocator>& x) { this->__swap_data(x); }
		void clear()
		{
			if (this->begin_) 
	...
	private : 
		...
		struct ConstructTransaction
		{
			...
			explicit ConstructTransaction(vector &v, size_type n) : v_(v),  pos_(v.end_), new_end_(v.end_ + n) {}
